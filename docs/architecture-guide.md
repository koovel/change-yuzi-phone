# Koove Phone koove手机架构说明

> 面向未来 AI 与开发者。目标：新增功能时，不需要重新从零理解项目结构，而是先遵守这里的模块边界、调用链、数据契约与维护规则。

## 1. 项目定位

Koove Phone 是 SillyTavern 第三方扩展，提供一个“小手机”样式的前端 UI。它不是独立后端，也不是数据源本身；它主要作为 SillyTavern 页面中的可视化前端壳层，依赖宿主和数据库插件暴露的 API。
...
```mermaid
graph TD
  A[Koove Phone 扩展入口] --> B[Bootstrap 挂载层]
  B --> C[Phone Core 运行时]
